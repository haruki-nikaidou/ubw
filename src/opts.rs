...
pub struct Opts {
    ...
    #[arg(
        help = "Add headers to the accept request",
        short = 'A',
        long = "accept-header"
    )]
...
impl FromStr for HeaderListItem {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (header_name, header_value) = s
            .split_once(':')
...
impl TryFrom<Vec<HeaderListItem>> for WrappedHeaderMap {
    ...
    fn try_from(value: Vec<HeaderListItem>) -> Result<Self, Self::Error> {
        value
            ...
            .map(|(name, value)| {
                let name = name
                    .parse::<hyper::http::header::HeaderName>()
                    .map_err(ParseHeaderListError::InvalidHeaderName)?;
                let value = value
                    .parse::<hyper::http::header::HeaderValue>()
                    .map_err(ParseHeaderListError::InvalidHeaderValue)?;
                Ok((name, value))
            })
            .try_fold(
                WrappedHeaderMap(HeaderMap::new()),
                |mut acc, item: Result<_, ParseHeaderListError>| {
                    let (name, value) = item?;
